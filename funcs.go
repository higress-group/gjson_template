...
func builtins() FuncMap {
	return FuncMap{
		...
		"gjson":    gjsonFunc, // Add gjson function
...
func gjsonFunc(path string, dot gjson.Result) gjson.Result {
...
func evalArgs(args []any) string {
	...
	if !ok {
		for i, arg := range args {
			// Handle gjson.Result type
			if result, ok := arg.(gjson.Result); ok {
				switch result.Type {
				case gjson.String:
			...
			// Handle other types using reflect
...
// gjsonPrintableValue returns a string representation of a gjson.Result value
func gjsonPrintableValue(v gjson.Result) (string, bool) {
	switch v.Type {
	case gjson.String:
